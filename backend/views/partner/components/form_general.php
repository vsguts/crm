...
$fields = [
    $form->field($model, 'parent_id')->widget('common\widgets\form\Select2', [
        'url' => ['partner/list', 'scope' => 'organizations'],
        'initValueText' => $model->parent ? $model->parent->extendedName : '',
        'relatedUrl' => ['update', 'id' => $model->parent_id],
    ]),
...
echo $form->field($model, 'publicTags')->widget('common\widgets\form\Tags', [
    'readonly' => !Yii::$app->user->can('public_tags_manage'),
]);

echo $form->field($model, 'personalTags')->widget('common\widgets\form\Tags', []);
